...
function sort(){ //做到我们的排序了，按照你当前点击的表头为依据排序
    ...
    ary = ary.sort(function (a,b){ //给数组排序，dom映射关系
        ...
        var curInn = a.cells[that.index].innerHTML
        var nextInn = b.cells[that.index].innerHTML
        var curInnNum = parseFloat(curInn)
        var nextInnNum = parseFloat(nextInn)
        if (isNaN(curInnNum) || isNaN(nextInnNum)){
            return (curInn.localeCompare(nextInn))*that.sortFlag
        }
        return (curInn-nextInn)*that.sortFlag; //排列倒叙如果每次在sort执行的时候，把return这个值乘以-1就ok
